...
export default function CountryPageClient() {
    ...
    // Декодируем slug в читаемое название
    const countryName = country ? country.charAt(0).toUpperCase() + country.slice(1).replace(/-/g, ' ') : 'Country';

    // Фильтруем рестораны по стране (используем data напрямую, а не data.results)
    const countryRestaurants = data?.filter(restaurant => {
        if (!restaurant.country) {
            return false;
        }
        const restaurantCountrySlug = createSlug(restaurant.country);
        return restaurantCountrySlug === country;
    }) || [];
                    ...
                    {countryRestaurants.length > 0 ? (
                        ...
                        <div className="text-center py-12">
                            ...
                            <p className="text-neutral-500">
                                We couldn&apos;t find any restaurants in {countryName} yet.
                            </p>
                        </div>
                    )}
                </Section>
